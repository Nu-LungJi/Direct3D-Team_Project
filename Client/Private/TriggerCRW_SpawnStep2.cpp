...
HRESULT CTriggerCRW_SpawnStep2::Initialize(void* pArg)
{
	return CPhysXCollisionProxyObject::Initialize(pArg);
}


void CTriggerCRW_SpawnStep2::Update(_float fTimeDelta)
{
	// 벡터를 순회하면서 타이머 업데이트
	if (false)
...
void CTriggerCRW_SpawnStep2::OnTriggerEnter(
				...
				case CMyMagicSquareStepController::PATTERN_EVENT::LINE_ISSUED:
...

					{
						...
						if (tmp == 0)
						{
							...
							if (auto pTrigger = CGameInstance::Get().GetGameObjectByHandleT<CTriggerCRW_SpawnStep2>(hTrigger))
							{
...
void CTriggerCRW_SpawnStep2::OnTriggerExit(
	E::CGameObject* pObj, const E::PX_ON_TRIGGER_DATA& info)
{
	DEBUG_LOG_STR(std::string("[PX][CTriggerCRW_SpawnStep2] Exit : ") +
		(pObj ? std::string{ pObj->GetObjectTag() } : "null") + "\n");
}

E::UPtr<CTriggerCRW_SpawnStep2> CTriggerCRW_SpawnStep2::Create()
{
	auto instance = E::ToUPtr(new CTriggerCRW_SpawnStep2{});
	if (FAILED(instance->InitializePrototype()))
		return nullptr;
	return instance;
}

E::UPtr<E::CPrototype> CTriggerCRW_SpawnStep2::Clone(void* pArg)
{
	auto instance = E::ToUPtr(new CTriggerCRW_SpawnStep2{ *this });
