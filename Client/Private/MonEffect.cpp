...
HRESULT CMonEffect::Initialize(void* pArg)
{
...
void CMonEffect::PriorityUpdate(E::_float fTimeDelta)
{
}

void CMonEffect::Update(E::_float fTimeDelta)
{
	if (m_fLifetime >= 0.f)
	{
...
void CMonEffect::LateUpdate(E::_float fTimeDelta)
{
	UpdatePhysicData();
	GetTransform().Update();
...
HRESULT CMonEffect::Render(ID3D11DeviceContext* pContext, const E::RENDER_CTX& ctx)
{
	return S_OK;
}

void CMonEffect::OnWake()
{
}

void CMonEffect::OnSleep()
{
	int x = 0;
}

void CMonEffect::OnCollisionEnter(CGameObject* pObj, const PX_ON_COLLISION_DATA& info)
{
	DEBUG_LOG_STR(std::string("[PX][TestPhysXBall] Collision Enter : ") +
		(pObj ? std::string{ pObj->GetObjectTag() } : "null") + "\n");
}

void CMonEffect::OnCollisionExit(CGameObject* pObj, const PX_ON_COLLISION_DATA& info)
{
	DEBUG_LOG_STR(std::string("[PX][TestPhysXBall] Collision Exit : ") +
		(pObj ? std::string{ pObj->GetObjectTag() } : "null") + "\n");
}

void CMonEffect::OnTriggerEnter(CGameObject* pObj, const PX_ON_TRIGGER_DATA& info)
{
	DEBUG_LOG_STR(std::string("[PX][TestPhysXBall] Trigger Enter : ") +
		(pObj ? std::string{ pObj->GetObjectTag() } : "null") + "\n");
}

void CMonEffect::OnTriggerExit(CGameObject* pObj, const PX_ON_TRIGGER_DATA& info)
{
	DEBUG_LOG_STR(std::string("[PX][TestPhysXBall] Trigger Exit : ") +
		(pObj ? std::string{ pObj->GetObjectTag() } : "null") + "\n");
}

E::UPtr<CMonEffect> CMonEffect::Create()
{
...
E::UPtr<E::CPrototype> CMonEffect::Clone(void* pArg)
{
	auto	pInstance = E::ToUPtr(new CMonEffect{ *this });
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CMonEffect");
		return nullptr;
	}
