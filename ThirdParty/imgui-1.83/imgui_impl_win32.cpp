...
static void ImGui_ImplWin32_CreateWindow(ImGuiViewport* viewport)
{
    ...
    data->Hwnd = ::CreateWindowEx(
        ...
        rect.left, rect.top, width, height,                                     // Window area
        parent_window, NULL, ::GetModuleHandle(NULL), NULL);                    // Parent window, Menu, Instance, Param
    if (data->Hwnd == NULL && parent_window != NULL)
    {
        data->Hwnd = ::CreateWindowEx(
    ...
    {
        char message[160];
        wsprintfA(message, "ImGui_ImplWin32_CreateWindow failed. GetLastError=%lu\n", ::GetLastError());
        ::OutputDebugStringA(message);
    }
    IM_ASSERT(data->Hwnd != NULL);
    data->HwndOwned = true;
