...
std::optional<CHandle>
CLightManager::Allocate_EffectLight(
	XMVECTOR _WorldPos,
	_float _Intensity,
	_float3 _Color,
	_float _Range,
	...
	_float3 _Velocity)
{
	...
	const size_t poolSize =
		m_pEffectLightPool.size();
	if (poolSize == 0)
	{
		DEBUG_LOG(
			"[EffectLight] Allocation failed: "
			"pool is empty.\n");
		return std::nullopt;
	}
	...
	for (size_t i = 0; i < poolSize; ++i)
	{
		const size_t circularIndex =
			(static_cast<size_t>(
				m_iLastAllocatedIndex) +
				1 + i) %
			poolSize;
		const auto& lightHandle =
			m_pEffectLightPool[circularIndex];
		if (!lightHandle)
			continue;
	...
	// LSY 변경: 사용 가능한 슬롯이 없을 때 기존 활성 라이트의 데이터를
	// 덮어쓰지 않고 명확하게 할당 실패를 반환한다.
	if (!selectedHandle || !selectedLight)
	{
		DEBUG_LOG(
			"[EffectLight] Allocation failed: "
			"no inactive light is available.\n");
		return std::nullopt;
	}
	...
	selectedLight->Set_LightPosition(
		worldPosition);
	selectedLight->Set_LightIntensity(
		_Intensity);
	selectedLight->Set_LightColor(
		_Color);
	selectedLight->Set_LightRange(
		_Range);
	selectedLight->Set_LightLifeTime(
		_LifeTime);
	selectedLight->Set_LightVelocity(
		_Velocity);

	// 모든 런타임 데이터를 적용한 후 렌더 대상으로 활성화한다.
	selectedLight->Set_LightActivateState(
		true);
