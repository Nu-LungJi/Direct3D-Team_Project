...
HRESULT CRenderer::RenderNonBlend()
{
	...
	const bool useHizCpuCulling =
		RenderContext.pass == RENDERPASS::DEFAULT &&
		m_bHasPrevHizBuffer &&
		m_pPrevHizBuffer != nullptr &&
		SUCCEEDED(UpdatePrevHizCpuMips());

	if (RenderContext.pass == RENDERPASS::DEFAULT)
	{
		m_iHizCpuTested = 0;
		m_iHizCpuCulled = 0;
    ...
    for (auto& pRenderObject : m_RenderObject[ETOUI(RENDERGROUP::NONBLEND)])
    {
        if (pRenderObject->HasRenderPass(RenderContext.pass))
        {
			if (useHizCpuCulling && pRenderObject->IsOcclusionCullable())
			{
				++m_iHizCpuTested;

				if (IsOcclusionCulledCPU(pRenderObject))
				{
					++m_iHizCpuCulled;
			...
			else if (m_bDrawOcclusionBounds && pRenderObject->IsOcclusionCullable())
			{
				DrawOcclusionBoundsDebug(pRenderObject, { 1.f, 1.f, 0.f, 1.f });
			}
...
HRESULT CRenderer::UpdatePrevHizCpuMips()
{
	if (m_pPrevHizBuffer == nullptr)
	{
		return E_FAIL;
	...
	const uint32_t mipCount = m_pPrevHizBuffer->GetMipCount();
	m_HizCpuMips.resize(mipCount);

	for (uint32_t mip = 0; mip < mipCount; ++mip)
	{
		auto& cpuMip = m_HizCpuMips[mip];
		if (FAILED(m_pPrevHizBuffer->ReadMipToCPU(mip, cpuMip.depths, cpuMip.width, cpuMip.height)))
		{
			m_HizCpuMips.clear();
			return E_FAIL;
		}
...
_float CRenderer::SampleHizCpuDepth(uint32_t mip, uint32_t x, uint32_t y) const
{
...
_bool CRenderer::IsOcclusionCulledCPU(const IRenderable* pRenderObject) const
{
	if (pRenderObject == nullptr || !pRenderObject->IsOcclusionCullable())
	{
		return false;
	}

	BoundingBox bounds{};
	if (!pRenderObject->GetOcclusionBounds(bounds)) // 여기서 받는 bounds는 이미 world space BoundingBox
	{
		return false;
	}

	XMFLOAT3 corners[BoundingBox::CORNER_COUNT]{};
	bounds.GetCorners(corners); // BoundingBox 8개 코너 얻기

	const auto clientSize = CGameInstance::Get().GetClientScreenSize();
	const _float screenWidth = clientSize.x;
	const _float screenHeight = clientSize.y;
	if (screenWidth <= 0.f || screenHeight <= 0.f)
	{
		return false;
	}

	// minX, minY, maxX, maxY = 오브젝트가 화면에서 차지하는 사각형
	// objectNearestDepth = 오브젝트에서 카메라에 가장 가까운 depth
	...
	// BoundingBox의 corner 점들을 투영 
	for (const auto& corner : corners)
	{
		const XMVECTOR worldPos = XMVectorSet(corner.x, corner.y, corner.z, 1.f);
		const XMVECTOR clipPos = XMVector4Transform(worldPos, RenderContext.matViewProj);
		...
		const _float screenY = (-ndcY * 0.5f + 0.5f) * screenHeight; // y는 부호 뒤집어줘야함
	...
	// ex : 선택된 mip이 240 * 135라면
	// mipScaleX = 240 / 1920 = 0.125
	// mipScaleY = 135 / 1080 = 0.125
	...
	// 선택된 mip의 전체 240x135를 도는 게 아니라, 그중 물체가 투영된 사각형 영역만 반복문돌리는 것
	
	//  물체의 화면 bounding rect가 선택된 mip에서 덮는 texel들만 비교
...
void CRenderer::DrawOcclusionBoundsDebug(const IRenderable* pRenderObject, const _float4& color) const
{
