	...
	MAPMESH_OCCLUSION_DATA& occlusionData)
{
	...
	auto& batch = m_FrameBatches[MODEL_RENDER_KEY{ model, renderFeature }];
...
void CMapMeshInstanceBatchCollector::ClearFrameBatches()
{
...
uint32_t CMapMeshInstanceBatchCollector::GetInstanceCount() const
{
	...
	for (const auto& batchEntry : m_FrameBatches)
