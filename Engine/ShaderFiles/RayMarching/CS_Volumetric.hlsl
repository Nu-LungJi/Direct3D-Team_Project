...
void CSMain(uint3 ID : SV_DispatchThreadID)
{
    ...
    if (Depth >= 1.f)
    {
        OUTPUT[ID.xy] = float4(SceneColor.rgb, 1.f);
        return;
    }
    ...
    float3 FinalColor = (SceneColor.rgb * LightTransmittance) + LightAccumulation * FogIntensity;
    OUTPUT[ID.xy] = float4(FinalColor, LightTransmittance);
    return;
}
