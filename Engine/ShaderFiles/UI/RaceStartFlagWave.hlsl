...
float4 PSMain(PS_IN input) : SV_Target
{
    ...
    const float freeEdgeWeight = smoothstep(0.04f, 0.96f, input.uv.x);
    ...
    warpedUV.x += cloudWave * 0.010f * freeEdgeWeight;
    warpedUV.y +=
        (cloudWave * 0.035f + ribbonWave * 0.010f) * freeEdgeWeight;
    ...
    maskValue = min(maskValue, flagMaskTexture.Sample(
        LinearClamp, warpedUV - float2(0.0f, maskTexel.y)).r);
    const float maskAlpha = smoothstep(0.32f, 0.62f, maskValue);
    ...
    const float clothLight = lerp(0.90f, 1.08f, cloud0);
