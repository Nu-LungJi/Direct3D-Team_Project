...
public:
	...
	*/
	struct ExecuteDesc
	{
		ID3D11Texture2D* pColorTex2D{};
		ID3D11Texture2D* pDepthTex2D{};
		ID3D11Texture2D* pMotionVectorTex2D{};
		ID3D11Texture2D* pOutputUAVTex2D{}; // D3D11_BIND_UNORDERED_ACCESS | D3D11_BIND_SHADER_RESOURCE
		...
		bool bCameraReset{ false }; //카메라 순간이동시 true
	};
	HRESULT Execute(const ExecuteDesc& desc);
