...
HRESULT CLightTerrain::InitializePrototype(void* pArg)
{
	m_pResTerrainVIBuffer = CGameInstance::Get().GetResourceFirst<CResTerrainVIBuffer>("LIGHT_SC", "VIBUFFER_Terrain");
	if (!m_pResTerrainVIBuffer)
	{
		return E_FAIL;
	}

	m_pResTerrainTexture2D = CGameInstance::Get().GetResourceFirst<CResTexture2D>("LIGHT_SC", "TEX2D_Terrain_Tile0");
...
HRESULT CLightTerrain::Initialize(void* pArg)
{
...
void CLightTerrain::PriorityUpdate(E::_float fTimeDelta)
{
}

void CLightTerrain::Update(E::_float fTimeDelta)
{
}
...
E::UPtr<CLightTerrain> CLightTerrain::Create()
{
	auto pInstance = E::ToUPtr(new CLightTerrain{});
	if (FAILED(pInstance->InitializePrototype()))
	{
		MSG_BOX("Failed to Created : CLightTerrain");
		return nullptr;
	}
	return  pInstance;
}

E::UPtr<E::CPrototype> CLightTerrain::Clone(void* pArg)
{
	auto	pInstance = E::ToUPtr(new CLightTerrain{ *this });
	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CLightTerrain");
		return nullptr;
	}
