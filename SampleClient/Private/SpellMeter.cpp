...
HRESULT CSpellMeter::Render(ID3D11DeviceContext* pContext, const E::RENDER_CTX& ctx)
{
	...
	{
		const auto& baseSrv = E::CGameInstance::GetConst().GetResourceFirst<E::CResTexture2D>(currentLevel, "UI_T_spellmeter_Generic");
		const auto& causticSrv = E::CGameInstance::GetConst().GetResourceFirst<E::CResTexture2D>(currentLevel, "T_WaterCaustics_Disorder_A");
		const auto& wispySrv = E::CGameInstance::GetConst().GetResourceFirst<E::CResTexture2D>(currentLevel, "VFX_T_WispyNoise_D");
		const auto& normalSrv = E::CGameInstance::GetConst().GetResourceFirst<E::CResTexture2D>(currentLevel, "VFX_T_Wavy_N");
